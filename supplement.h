...
16-19 hashchain ID number (uninsteresting)
20-(end of signature) (k+1) 20-byte secrets

For our ORIGINAL specific pcap file:
data is 4 bytes, so:
16-19 data
...
For FUTURE pcap files:
data is 4 bytes, so:
16-19 data
20-27 tvots timestamp
28-31 tvots seq. num
32-35 tvots epoch ID
36-39 hashchain ID num
40-59 "salt" secret (only changes when the epoch ID changes)
60-79 secret
80-99 secret
 .
 .
...
*/
#define MAX_GPU_THREADS  45056
#define TARGET_NUMBER_PACKETS 5900
#define MD_LENGTH 20
#define TARGET_PACKET_LENGTH 312
#define K_SECRET_POSITION 60
#define NUMBER_K_SECRETS 12
...
#define HASHCHAIN_ID_LENGTH 4
#define HASHCHAIN_ID_POSITION 36
#define SALT_SECRET_LENGTH 20
#define SALT_SECRET_POSITION 40
...
handleDataV2(u_char* dataPool, u_int dataLength, u_int dataPoolSize, u_int* secretPool,
  u_int spoolSize, u_int bValue, u_int maxIDs,  u_int throwAway, u_int* ret, struct btimes* times);

extern "C" bool
handleDataV3(u_int seed, u_int dataPoolSize, u_int* secretPool,
  u_int spoolSize, u_int bValue, u_int maxIDs,  u_int throwAway, u_int* ret, struct btimes* times);

extern "C" bool
handleDataPerSecond(u_char* dataPool, u_int dataLength, u_int dataPoolSize, u_int* secretPool,
  u_int spoolSize, u_int bValue, u_int maxIDs,  u_int throwAway, u_int* ret, struct btimes* times);

extern "C" bool
hashesPerSecond(u_int* numHashes, struct btimes* times);

extern "C" bool
hashFindPerSecond(u_int* numHashes, struct btimes* times); 
...
extern "C" bool
sha1KernelPerSecond(u_char* src, int width, int height, u_char* dst, struct btimes *times, u_int* hashes);
/*
