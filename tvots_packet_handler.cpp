...
TVOTS_Packet::TVOTS_Packet( const u_char* packet){
  ...
  //hashchain id
  this->hashchain_ID = new u_char[EPOCH_ID_LENGTH];
  memcpy(this->hashchain_ID, packet + HASHCHAIN_ID_POSITION, HASHCHAIN_ID_LENGTH);
...
u_char* TVOTS_Packet::getHashChainID(){
  return this->hashchain_ID;
}
